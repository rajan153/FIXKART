        ...
        <div className="flex-1 overflow-y-auto p-2 custom-scrollbar">
          <ul className="space-y-1">
            {SIDEBAR_LINKS.map((linkName, index) => {
              const catSlug =
                INVENTORY_DATA.find((c) => c.title === linkName)?.slug || "";
...
const ProductCard = React.memo(function ProductCard({
  ...
  return (
    <div className="flex flex-col items-center group cursor-pointer w-full h-full bg-white rounded-lg p-2 transition-all hover:bg-white">
      ...
      <span
        className={`text-[13px] text-center leading-snug font-medium px-1 line-clamp-2 ${
          isAvailable
            ? "text-gray-600 group-hover:text-[#00529b]"
